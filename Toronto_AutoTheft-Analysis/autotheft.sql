...
-- -- What is the percentage break down of thefts by location
-- SELECT
-- 	location_type,
-- 	ROUND(COUNT(*) / 58441::numeric * 100,  2) AS loc_pct
-- FROM auto_theft_open_data
-- GROUP BY location_type
-- ORDER BY loc_pct DESC;

-- -- Show the percentage increase/decrease of thefts in single homes per year
-- WITH single_home_thefts AS (
-- 	SELECT
-- 		occ_year,
-- 		location_type,
-- 		COUNT(*) AS thefts,
-- 		LAG(COUNT(*)) OVER(ORDER BY occ_year) AS prev_thefts
-- 	FROM auto_theft_open_data
-- 	WHERE location_type = 'Single Home, House (Attach Garage, Cottage, Mobile)'
-- 		AND occ_year >= 2014
-- 	GROUP BY occ_year, location_type
-- 	ORDER BY occ_year
-- )
...
-- 	location_type,
-- 	thefts,
-- 	ROUND((thefts - prev_thefts) / thefts::numeric * 100, 2) AS pct_inc_dec
-- FROM single_home_thefts
...
-- SELECT
-- 	occ_year,
-- 	(residential + commercial + other) AS total_thefts,
-- 	residential,
-- 	commercial,
-- 	other,
