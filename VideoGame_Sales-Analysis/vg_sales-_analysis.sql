...
-- What platform is the best-selling for each region
WITH regional_console_sales AS (
	SELECT
		console,
		SUM(na_sales) AS total_na_sales,
		SUM(jp_sales) AS total_jp_sales,
...
SELECT *
FROM regional_console_sales
WHERE total_na_sales = (SELECT MAX(total_na_sales) FROM regional_console_sales)
	OR total_jp_sales = (SELECT MAX(total_jp_sales) FROM regional_console_sales)
	OR total_pal_sales = (SELECT MAX(total_pal_sales) FROM regional_console_sales)
	OR total_other_sales = (SELECT MAX(total_other_sales) FROM regional_console_sales)
